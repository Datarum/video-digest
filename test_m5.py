...
def make_test_video(path: Path, duration: int = 30) -> Path:
    """Generate a synthetic color-bar test video using ffmpeg."""
    cmd = [
        "ffmpeg", "-y",
        "-f", "lavfi",
...
def _make_solid(size=128) -> "Image":
    from PIL import Image
    return Image.new("RGB", (size, size), (128, 128, 128))


def _make_checkerboard(size=128) -> "Image":
    """High-frequency checkerboard — pHash very different from a solid image."""
    import numpy as np
    from PIL import Image
...
def test_dedup_logic():
    ...
    # Two identical images → should be duplicates (distance = 0)
    h1 = imagehash.phash(_make_solid())
    h2 = imagehash.phash(_make_solid())
    assert _is_duplicate(h2, [h1], threshold=8), "Identical images should be duplicates"

    # Structurally very different image (checkerboard vs solid) → distance ~31, not duplicate
    h3 = imagehash.phash(_make_checkerboard())
    assert not _is_duplicate(h3, [h1], threshold=8), "Checkerboard vs solid should not be duplicate"
...
def test_candidate_selection():
    ...
    # When segments <= max_candidates, returns all
...
def test_extract_frames():
    ...
    # Build 3 segments covering 0-10, 10-20, 20-30 seconds
    segments = [
    ...
    for f in frames:
        ...
        assert f.path.exists(), f"Frame file missing: {f.path}"

    assert len(frames) >= 1, "Should extract at least 1 frame"
    assert all(isinstance(f, Frame) for f in frames)
    assert frames == sorted(frames, key=lambda x: x.timestamp), "Frames should be chronological"
