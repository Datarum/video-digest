...
def make_fake_frame(ts: float, idx: int, frames_dir: Path) -> Frame:
    """Write a tiny JPEG and return a Frame pointing to it."""
...
def test_nearest_frame():
    ...
    assert _nearest_frame(frames, 0) == frames[0]      # closest to 0s → 10s frame
    assert _nearest_frame(frames, 125) == frames[1]    # closest to 125s → 130s frame
    assert _nearest_frame(frames, 400) == frames[2]    # closest to 400s → 310s frame
...
def test_save_markdown_structure():
    ...
    assert "Introduction to Python" in md  # key point
    assert "[00:00]" in md
    assert "[02:00]" in md
    assert "[05:00]" in md
...
def test_save_markdown_frames_copied():
    print("\n=== M7: save_markdown — frames copied to output/frames/ ===")
    frames_dir = TMP / "frames_copy_src"
    frames_dir.mkdir()
    frames = [make_fake_frame(ts, i, frames_dir) for i, ts in enumerate([5.0, 130.0])]
    summary = make_summary(frames[:2])

    out = TMP / "output_copy" / "summary.md"
    save_markdown(summary, out, video_id="abc123")

    # Frames should be copied into output/copy/frames/
    copied = list((TMP / "output_copy" / "frames").glob("*.jpg"))
    assert len(copied) == 2, f"Expected 2 copied frames, got {len(copied)}"
    print(f"  {len(copied)} frame(s) copied to output/frames/: OK")

    # Markdown should reference relative paths
    md = out.read_text()
    assert "frames/" in md
    print("  Markdown uses relative frame paths: OK")
...
def test_save_markdown_no_frames():
    ...
    assert "## Overview" in md
    assert "## Key Points" in md
    assert "![" not in md  # no image tags
    print("  Text-only markdown (no images): OK")


def test_markdown_preview():
    print("\n=== M7: Markdown preview ===")
    frames_dir = TMP / "frames_preview"
    frames_dir.mkdir()
    frames = [make_fake_frame(ts, i, frames_dir) for i, ts in enumerate([5.0, 130.0, 310.0])]
    summary = make_summary(frames)
    out = TMP / "output_preview" / "summary.md"
    save_markdown(summary, out, video_id="dQw4w9WgXcQ", channel="Test Channel", duration_str="08:30")

    md = out.read_text()
    print()
    for line in md.splitlines()[:35]:
        print(f"  {line}")
    if len(md.splitlines()) > 35:
...
if __name__ == "__main__":
    ...
    test_save_markdown_frames_copied()
